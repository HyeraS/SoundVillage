...
test('scene query defaults to the hub and only allows direct Animal access in mock mode', () => {
  assert.equal(getPrototypeScene(''), 'hub')
  assert.equal(getPrototypeScene('mock=1'), 'hub')
  assert.equal(getPrototypeScene('mock=1&scene=hub'), 'hub')
  assert.equal(getPrototypeScene('scene=unknown'), 'hub')
  assert.equal(getPrototypeScene('scene=animal'), 'hub')
  assert.equal(getPrototypeScene('mock=1&scene=animal'), 'animal')
  assert.equal(getPrototypeScene('scene=music'), 'music')
  assert.equal(getPrototypeScene('mock=1&scene=music'), 'music')
})
