...
import {
  BASE_WORLD_BOUNDS,
  ANIMAL_ENVIRONMENT_CONFIG,
  ANIMAL_EXIT_TARGET,
  ANIMAL_PLAYER_START,
  ANIMAL_WORLD_COLLIDERS,
...
test('Animal uses the same exactly doubled area without scaling player or interaction contracts', () => {
  const oldArea = width(BASE_WORLD_BOUNDS) * height(BASE_WORLD_BOUNDS)
  assert.ok(Math.abs((width(WORLD_BOUNDS) * height(WORLD_BOUNDS)) / oldArea - 2) < 1e-12)
  assert.equal(ZONE_LINEAR_SCALE, Math.SQRT2)
  assert.equal(PLAYER_MOVE_SPEED, 4.1)
  assert.equal(SOUND_INTERACTION_RADIUS, 1.62)
  assert.equal(ANIMAL_EXIT_TARGET.interactionRadius, 1.9)
  assert.ok(ANIMAL_ENVIRONMENT_CONFIG.groundTopRadius > 13.2)
})

test('Animal spawn, exit, and all 15 deterministic Orb slots are walkable and avoid the pond', () => {
  const placements = createSoundPlacements(Array.from({ length: 15 }, (_, index) => ({ sound_id: `Animal_${index + 1}` })), 'Animal')
  assert.equal(isPositionWalkable(positionOf(ANIMAL_PLAYER_START), PLAYER_RADIUS, WORLD_BOUNDS, ANIMAL_WORLD_COLLIDERS), true)
  assert.equal(isPositionWalkable(positionOf(ANIMAL_EXIT_TARGET.position), PLAYER_RADIUS, WORLD_BOUNDS, ANIMAL_WORLD_COLLIDERS), true)
  const pond = ANIMAL_WORLD_COLLIDERS.find(item => item.id === 'animal-pond')
  for (const placement of placements) {
    assert.equal(isPositionWalkable(positionOf(placement.position), PLAYER_RADIUS, WORLD_BOUNDS, ANIMAL_WORLD_COLLIDERS), true)
    assert.equal(Math.abs(placement.position[0] - pond.position[0]) <= pond.size[0] / 2 && Math.abs(placement.position[2] - pond.position[1]) <= pond.size[1] / 2, false)
  }
})
