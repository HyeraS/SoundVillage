...
export default function MusicExitOverlay({ onConfirm, onClose }) {
  useEffect(() => {
  ...
  return (
    <section className={styles.villageEntryOverlay} role="dialog" aria-modal="true" aria-labelledby="music-exit-title">
      <div className={styles.villageEntryCard} style={{ '--village-color': '#E88C72' }}>
        <span className={styles.villageEntryIcon} aria-hidden="true">↩</span>
        <span className={styles.eyebrow}>MUSIC VILLAGE EXIT</span>
        <h2 id="music-exit-title">중앙 광장으로 돌아갈까요?</h2>
        <p>Music 진행 상태를 유지한 채 입구 근처의 중앙 광장으로 돌아갑니다.</p>
